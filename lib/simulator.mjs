export function simulateScenario({ basePrice, tankLiters, scenario, referenceDate, fuel }) {
  ...
  if (model.validFrom && referenceDate < model.validFrom) return { available:false, referenceDate, source:scenario.source, verifiedAt:scenario.verifiedAt, evidence:scenario.evidence };
  if (model.validTo && referenceDate > model.validTo) return { available:false, referenceDate, source:scenario.source, verifiedAt:scenario.verifiedAt, evidence:scenario.evidence };
  if (Array.isArray(model.fuels) && !model.fuels.includes(fuel)) return { available:false, referenceDate, source:scenario.source, verifiedAt:scenario.verifiedAt, evidence:scenario.evidence };
